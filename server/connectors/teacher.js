...
function sendList(req, res) {
    ...
    rtn = component.teacher('list', root);
    rtn.action = {};
    rtn.action.template = listActions('teacher',root);
...
function sendItem(req, res, id) {
    ...
    rtn = component.teacher('read', id, root);
    rtn.action = {};
    rtn.action.template = listActions('teacher',root);
    doc = {action:{link:[]}};
    doc.action.link = utils.pageActions(root);
...
function addItem(req, res) {
    ...
    req.on('end', function() {
        try {
            msg = qs.parse(body);
            list = component.teacher('add', msg);
...
function updateItem(req, res, id) {
    ...
    req.on('end', function() {
        try {
            msg = qs.parse(body);
            list = component.teacher('update', id, msg);
            doc = {code:200, doc:list};
        }
        catch(ex) {
            doc = utils.errorDoc(req, res, 'Server Error', 500);
...
function removeItem(req, res, id) {
    ...
    doc = component.teacher('remove', id, root);
    doc = '';
...
function listActions(name, root) {
    ...
    data = [];
    data.push({name:'teacherName',value:'',prompt:'teacherName'});
